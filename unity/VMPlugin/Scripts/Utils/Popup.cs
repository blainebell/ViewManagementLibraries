...
public class Popup {
	...
	public int List(Rect box, GUIContent[] items, GUIStyle boxStyle, GUIStyle listStyle, List<Rect> allRects) {
		...
		Rect clickedRect = new Rect(box.x,box.y,box.width,box.height);
		allRects.Add (clickedRect);
		if(GUI.Button(clickedRect,items[selectedItemIndex])) {
	...
	public void SetSelectedItemIndex(int sel)
	{
