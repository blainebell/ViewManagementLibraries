...
/**
 * This class implements a one-dimensional Interval Tree data structure. It
 * has the flexability to be used for multi-dimensional Interval Trees
 * depending on what arguments, classes are passed in the constructor.
 * <p>
 * @author Blaine Bell
 * @since JDK1.1
 *
...
public class IntervalTree implements Cloneable, Serializable {
  ...
  private void readObject(java.io.ObjectInputStream in)
	...
	//	System.out.println("nC=" + nC + "\tminMeth=" + minMeth + "\tmaxMeth=" +maxMeth + "\tReturn Type=" + minMeth.getReturnType().toString() + "\tminMeth.getReturnType().toString()=" + minMeth.getReturnType().toString());
	if (nC==null || minMeth== null || maxMeth==null || minMeth.getReturnType().toString().compareTo("int")!=0){
  ...
  public void printObjects(PrintStream ps){
    if (rootNode!=null)
      rootNode.printSubTreeObjects(ps,0);
  }
    ...
    public boolean isEnclosedBy(Rectangle2i r){
	boolean b=isEnclosedBy(r,new NeededCriteria());
	//	System.out.println("isEnclosedBy returned=" + b);
	return(b);
    }
    public boolean isEnclosedBy(Rectangle2i r, NeededCriteria neededCriteria){
	...
	int rmin=0, rmax=0;
	try {
	    rmin = ((Integer)nodeMinMethod.invoke(r,null)).intValue();
	    rmax = ((Integer)nodeMaxMethod.invoke(r,null)).intValue();
  ...
  /* Checks to see if any rectangles overlap a given rectangle, not including edges */
  public boolean isOverlapped(Object r){
    return (windowQuery(r,new NeededCriteria(), false, true).size()>0);
  }
    public Vector windowQuery(Object r, NeededCriteria neededCriteria, boolean inclusive){
      return (windowQuery(r,neededCriteria, inclusive, false));
    }
    public Vector windowQuery(Object r, NeededCriteria neededCriteria, boolean inclusive, boolean justGetOne){
	if (!nodeClass.isInstance(r)){
	    throw new IllegalArgumentException();
	}
	int rmin=0, rmax=0;
	...
	while (!searchStack.empty()){
		...
		if (!currentNode.secondaryStructure.isEmpty()){
		    Rectangle2i rect = new Rectangle2i();
		    rect.setRect(rmax,Integer.MAX_VALUE,rmax,Integer.MAX_VALUE);
    ...
    public void addRectangle(Rectangle2i rs){
	...
	double rcenter=0.;
	try {
	    rmin = ((Integer)nodeMinMethod.invoke(rs,null)).intValue();
	    rmax = ((Integer)nodeMaxMethod.invoke(rs,null)).intValue();
	...
	while (itn!= null && !deleted){
		...
		if (!deleted){
		    ...
		    System.out.println("IntervalTree.deleteRectangle didn't delete r=" + r);
		    return (false);
		    //		    System.exit(-1);
		}
	    }
	...
	public void printNodeObjects(PrintStream ps){
	  if (active){
	    secondaryStructure.printObjects(ps);
	  }
	}
	...
	public void printSubTreeObjects(PrintStream ps, int depth){
	    if (primaryStructure[0]!=null)
		primaryStructure[0].printSubTreeObjects(ps,depth+1);
	    printNodeObjects(ps);
	    if (primaryStructure[1]!=null)
		primaryStructure[1].printSubTreeObjects(ps,depth+1);
	}
	...
	public void addRectangle (Rectangle2i r){
